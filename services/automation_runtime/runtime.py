...
class AutomationRuntime:
    ...
    def __init__(
        ...
        *,
        ...
        self.log = logger.bind(component="AutomationRuntime", service=name)
    ...
    def start(self) -> None:
        ...
        self._thread = threading.Thread(target=self._run_loop, daemon=True, name="automation-runtime")
        self._thread.start()
    ...
    def _cmd_start_chain(self, payload: dict[str, Any]) -> None:
        ...
        if fn is None:
            ...
            return
        ctx = AutomationContext(chain_id=chain_key, worker_bus=self.worker_bus, bridge=self.bridge, state=self.bridge, send_cmd=self.send_cmd)

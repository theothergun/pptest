...
from services.automation_runtime.apis.values_api import ValuesApi
from services.automation_runtime.apis.vars_api import VarsApi
...
class AutomationContext:
    """
    Internal engine context for one running automation program instance.
    ...
    def __init__(
        ...
        logger.bind(component="AutomationContext", chain_id=self.chain_id).debug("created")
    ...
    @property
    def public(self) -> "PublicAutomationContext":
        """Public, script-safe API wrapper."""
        if self._public is None:
            self._public = PublicAutomationContext(self)
        return self._public
...
class PublicAutomationContext:
    """Stable, script-facing context (public API)."""

    def __init__(self, ctx: AutomationContext) -> None:
        self._ctx = ctx

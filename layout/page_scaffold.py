...
def build_page(
    ...
    *,
    ...
    - If ctx.action_bar is present and show_action_bar=True: it is pinned at the bottom

    Pages only provide `content(parent)`; no repeated flex/overflow code.
    """

    def has_action_bar() -> bool:
        # If action_bar exists but has no actions, you likely don't want to show it.
        # If your ActionBar has a "has_actions" property/method, use it here.
    ...
    with container:
        ...
        with ui.column().classes("w-full h-full min-h-0 min-w-0"):
            ...
            with ui.column().classes(
                f"w-full flex-1 min-h-0 min-w-0 overflow-auto {content_padding_classes}"
            ) as content_area:
            ...
            if show_action_bar and has_action_bar():
                with ui.column().classes("w-full shrink-0"):
                    ...
                    #with ui.card().classes("w-full no-border"):
                    if actions_title:
                        ui.label(actions_title).classes("text-sm text-gray-500")
                    # Render the action bar content into a full-width container
                    ctx.action_bar.render(ui.column().classes("w-full mb-1"))

                if on_action_clicked is not None:
                    # router creates ctx.bus + ctx.action_bar for each route; just subscribe:
                    if ctx.bus is None:

...
def build_page(
	...
	*,
	...
	show_action_bar_separator: bool = True,
	...
	with container:
		with ui.column().classes("w-full h-full min-h-0 min-w-0 px-1 pt-1 pb-2 gap-1"):
			if title:
				ui.label(title).classes("text-2xl font-bold text-[var(--text-primary)] px-1")
			...
			if show_action_bar and has_action_bar():
				col_cls = "w-full shrink-0 p-1%s"% " border-t" if show_action_bar_separator else ""
				with ui.column().classes(col_cls) as action_bar:
					if show_action_bar_separator:
						action_bar.style("border-color:var(--input-border);")
					if actions_title:
